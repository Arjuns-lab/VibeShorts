...
interface UploadProps {
  onPost: (data: { videoFile: File; caption: string; textOverlays: TextOverlay[], filterClass: string; startTime: number; endTime: number; }) => void;
  onCancel: () => void;
}
...
const Upload: React.FC<UploadProps> = ({ onPost, onCancel }) => {
  ...
  const handlePost = () => {
    ...
    // Simulate a more realistic upload progress
    uploadIntervalRef.current = window.setInterval(() => {
        setUploadProgress(prev => {
            ...
            if (newProgress >= 100) {
                ...
                // Add a small delay at 100% to simulate processing
                setTimeout(() => {
                    onPost({ videoFile, caption, textOverlays, filterClass: selectedFilter, startTime, endTime });
                }, 500);
      ...
      {isUploading && (
        <div className="absolute inset-0 bg-black/70 backdrop-blur-sm z-40 flex flex-col justify-center items-center p-8 text-center">
          <div className="flex items-center justify-center gap-4">
            ...
            <h2 className="text-2xl font-black text-white font-display">Uploading Your Vibe...</h2>
          </div>
          <p className="text-white/70 text-sm mt-2 mb-4">Please wait, this may take a moment.</p>
          ...
          <p className="text-white font-bold text-lg mt-2">{Math.round(Math.min(uploadProgress, 100))}%</p>
          <button 
